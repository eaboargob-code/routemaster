"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.onPassengerStatusChange = void 0;
// functions/index.ts
var functions = require("firebase-functions");
var admin = require("firebase-admin");
admin.initializeApp();
exports.onPassengerStatusChange = functions.firestore
    .document('trips/{tripId}/passengers/{studentId}')
    .onWrite(function (change, ctx) { return __awaiter(void 0, void 0, void 0, function () {
    var after, before, _a, tripId, studentId, schoolId, studentName, studentSnap, s, parents, titleMap, title, body, batch;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                after = change.after.exists ? change.after.data() : null;
                before = change.before.exists ? change.before.data() : null;
                if (!after)
                    return [2 /*return*/];
                if ((before === null || before === void 0 ? void 0 : before.status) === after.status)
                    return [2 /*return*/]; // no real change
                _a = ctx.params, tripId = _a.tripId, studentId = _a.studentId;
                schoolId = after.schoolId || null;
                studentName = after.studentName || null;
                if (!!studentName) return [3 /*break*/, 2];
                return [4 /*yield*/, admin.firestore().doc("students/".concat(studentId)).get()];
            case 1:
                studentSnap = _b.sent();
                if (studentSnap.exists) {
                    s = studentSnap.data();
                    studentName =
                        s.name ||
                            s.displayName ||
                            ([s.firstName, s.lastName].filter(Boolean).join(' ')) ||
                            null;
                }
                _b.label = 2;
            case 2:
                // 🔹 Last fallback = just show UID
                if (!studentName)
                    studentName = studentId;
                return [4 /*yield*/, admin.firestore()
                        .collection('parentStudents')
                        .where('studentIds', 'array-contains', studentId)
                        .get()];
            case 3:
                parents = _b.sent();
                if (parents.empty)
                    return [2 /*return*/];
                titleMap = {
                    boarded: "On Bus 🚌",
                    dropped: "Dropped Off ✅",
                    absent: "Marked Absent 🚫",
                    pending: "Awaiting Check-in 🕓",
                };
                title = titleMap[after.status] || "Update";
                body = "Student ".concat(studentName, " is ").concat(after.status, ".");
                batch = admin.firestore().batch();
                parents.forEach(function (p) {
                    var inboxRef = admin.firestore()
                        .collection("users")
                        .doc(p.id)
                        .collection("inbox")
                        .doc();
                    batch.set(inboxRef, {
                        title: title,
                        body: body,
                        createdAt: admin.firestore.FieldValue.serverTimestamp(),
                        read: false,
                        data: {
                            kind: "passengerStatus",
                            schoolId: schoolId,
                            tripId: tripId,
                            studentId: studentId,
                            studentName: studentName, // ✅ now always stored
                            status: after.status,
                        },
                    });
                });
                return [4 /*yield*/, batch.commit()];
            case 4:
                _b.sent();
                return [2 /*return*/];
        }
    });
}); });
