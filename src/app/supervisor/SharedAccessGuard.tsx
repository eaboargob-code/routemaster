    ...
    return (
         <header className="sticky top-0 flex h-16 items-center gap-4 border-b bg-background px-4 md:px-6 z-50">
            <nav className="hidden flex-col gap-6 text-lg font-medium md:flex md:flex-row md:items-center md:gap-5 md:text-sm lg:gap-6">
                <a href="/admin" className="flex items-center gap-2 text-lg font-semibold md:text-base">
...
export function SharedAccessGuard({ children }: { children: ReactNode }) {
  ...
  const isAllowed = profile.role === 'admin' || profile.role === 'supervisor';
  
  if (!isAllowed) {
    return <AccessDeniedScreen message="Access Denied" details={`Your role ('${profile.role}') is not authorized to view this page.`} />;
  }
